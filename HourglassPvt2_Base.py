...
class HourglassPvt_v2(nn.Module):
    def __init__(self, img_size=384, patch_size=None, in_chans=3, num_classes=1000,
                 embed_dims=[64, 128, 256, 512], num_heads=[1, 2, 4, 8],
                 mlp_ratios=[4, 4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], num_stages=5, linear=False):
        super().__init__()
        self.depths = depths
        ...
        self.DSAM_blocks = nn.ModuleList()
        for i in range(num_stages):
            if i >= num_stages - 3:
                self.DSAM_blocks.append(
                    DSAM(embed_dims[i], num_heads=max(1, num_heads[i] // 2))
                )
            else:
                self.DSAM_blocks.append(nn.Identity())
...
class DSAM(nn.Module):
    ...
    def forward(self, x):
        ...
        for head_idx in range(self.num_heads):
            k_values_for_head = k_values[:, head_idx, 0, 0]
