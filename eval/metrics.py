...
class Smeasure(object):
    def __init__(self, alpha: float = 0.5):
        ...
        self.test_camo_mask_path = '../../COD_dataset/test_set/CAMO/gt/'
        self.test_cod10k_mask_path = '../../COD_dataset/test_set/COD10K/gt/'
        self.train_mask_path = '../../COD_dataset/train_set/gt/'
    ...
    def step_by_train(self, pred: np.ndarray, gt_name, set_name):
        test_mask_path = ''
        if set_name == 'CAMO':
            test_mask_path = self.test_camo_mask_path
        elif set_name == 'COD10K':
            test_mask_path = self.test_cod10k_mask_path

        gt = Image.open(test_mask_path + gt_name).convert('L')
        if not pred.shape == gt.size:
            gt = gt.resize(pred.shape)
        gt = np.array(gt)
        self.step(pred, gt)

    def step_by_search(self, pred: np.ndarray, gt_name):
        gt = Image.open(self.train_mask_path + gt_name).convert('L')
